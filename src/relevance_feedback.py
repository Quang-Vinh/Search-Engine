...
class Rocchio():
    ...
    def __init__(self, index: InvertedIndex, alpha: float = 0.8, beta: float = 0.3, gamma: float = 0.1):
    ...
    def update_query_vector(self, query_vector: List[Tuple[str, float]], relevant_doc_ids: list, non_relevant_doc_ids: list) -> List[Tuple[str, float]]:
        ...
        relevant_vectors = np.array([self.index.get_docID_vector(doc_id) for doc_id in relevant_doc_ids])
        non_relevant_vectors = np.array([self.index.get_docID_vector(doc_id) for doc_id in non_relevant_doc_ids])
